...
function create() {
  ...
  for (let x = 0; x < 4; x++) {
    arr.push([
      {
        x: window.innerWidth / 2 - Math.cos((Math.PI * x) / 2) * 250,
        y: window.innerHeight / 2 - Math.sin((Math.PI * x) / 2) * 250,
      },
      {
        x: window.innerWidth / 2 + Math.cos((Math.PI * x) / 2) * 250,
        y: window.innerHeight / 2 + Math.sin((Math.PI * x) / 2) * 250,
...
function draw() {
  ...
  for (let x = 0; x < 4; x++) {
    // context.strokeStyle = `rgb(${255 - (255 / 4) * x},${Math.random()*255},${(255 / 4) * x})`;
    for (let i = 0; i < arr[x].length - 1; i++) {
      context.strokeStyle = `rgb(${255 - (255 / 4) * x},${
        (arr[x][i].x + arr[x][i].y) / 8
      },${(255 / 4) * x})`;
